...
Verify node behaviour and debug log when launching atcoind in these cases:

1. `atcoind` with no -asmap arg, using /16 prefix for IP bucketing

2. `atcoind -asmap=<absolute path>`, using the unit test skeleton asmap

3. `atcoind -asmap=<relative path>`, using the unit test skeleton asmap

4. `atcoind -asmap/-asmap=` with no file specified, using the default asmap

5. `atcoind -asmap` restart with an addrman containing new and tried entries

6. `atcoind -asmap` with no file specified and a missing default asmap file

7. `atcoind -asmap` with an empty (unparsable) default asmap file
...
class AsmapTest(BitcoinTestFramework):
    ...
    def test_without_asmap_arg(self):
        self.log.info('Test atcoind with no -asmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0)

    def test_noasmap_arg(self):
        self.log.info('Test atcoind with -noasmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0, ["-noasmap"])

    def test_asmap_with_absolute_path(self):
        self.log.info('Test atcoind -asmap=<absolute path>')
        self.stop_node(0)
    ...
    def test_asmap_with_relative_path(self):
        self.log.info('Test atcoind -asmap=<relative path>')
        self.stop_node(0)
        name = 'ASN_map'
    ...
    def test_default_asmap(self):
        ...
        for arg in ['-asmap', '-asmap=']:
            self.log.info(f'Test atcoind {arg} (using default map file)')
            self.stop_node(0)
    ...
    def test_asmap_interaction_with_addrman_containing_entries(self):
        self.log.info("Test atcoind -asmap restart with addrman containing new and tried entries")
        self.stop_node(0)
        shutil.copyfile(self.asmap_raw, self.default_asmap)
