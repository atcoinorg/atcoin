...
const char * const BITCOIN_CONF_FILENAME = "atcoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; // atcoin-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\ATCOIN
    //   new: C:\Users\Username\AppData\Local\ATCOIN
    // macOS: ~/Library/Application Support/ATCOIN
    // Unix-like: ~/.atcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "ATCOIN";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "ATCOIN";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/ATCOIN";
#else
    // Unix-like
