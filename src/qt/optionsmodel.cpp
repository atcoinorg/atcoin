...
/** Call node.updateRwSetting() with ATCOIN 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because ATCOIN 22.x releases try to read these specific settings as
        ...
        // instead of strings, because atcoin 22.x will not try to read these.
...
//! Convert enabled/size values to atcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from atcoin -prune setting.
...
//! Get pruning size value to show in GUI from atcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    if (!settings.contains("DisplayBitcoinUnit")) {
        settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(BitcoinUnit::ATCOIN));
    }
    QVariant unit = settings.value("DisplayBitcoinUnit");
    if (unit.canConvert<BitcoinUnit>()) {
        m_display_bitcoin_unit = unit.value<BitcoinUnit>();
    } else {
        m_display_bitcoin_unit = BitcoinUnit::ATCOIN;
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if atcoin.conf value
