...
The ATCOIN Core project supports the following vcpkg triplets:
- `x64-windows` (both CRT and library linkage is dynamic)
- `x64-windows-static` (both CRT and library linkage is static)

To facilitate build process, the ATCOIN Core project provides presets, which are used in this guide.
...
vcpkg installation during the configuration step might fail for various reasons unrelated to ATCOIN Core.
...
can occur if your local ATCOIN Core repository path contains spaces, you can override the vcpkg install directory
