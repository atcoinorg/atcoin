...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing atcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='atcoin-cli', help='atcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='ATCOIN address to which the faucet should send')
...
parser.add_argument('bitcoin_cli_args', nargs='*', help='Arguments to pass on to atcoin-cli (default: -signet)')
