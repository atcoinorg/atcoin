# CJDNS support in ATCOIN Core

It is possible to run ATCOIN Core over CJDNS, an encrypted IPv6 network that
...
done outside of ATCOIN Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the ATCOIN network and individual nodes.
...
before it will work with your ATCOIN Core node. This is described in steps
...
## Run ATCOIN Core with CJDNS

Once you are connected to the CJDNS network, the following ATCOIN Core
...
When enabled, this option tells ATCOIN Core that it is running in an
...
IPv6 local network. This helps ATCOIN Core perform better address management:
...
CJDNS support was added to ATCOIN Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `atcoin-cli -addrinfo` to see the
...
## CJDNS-related information in ATCOIN Core

There are several ways to see your CJDNS address in ATCOIN Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `atcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `atcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `atcoin-cli help getnodeaddresses` for details.
