...
Below are some notes on how to build ATCOIN Core for Windows.

The options known to work for building ATCOIN Core on Windows are:
...
Note that for WSL the ATCOIN Core source path MUST be somewhere in the default mount file system, for
example /usr/src/atcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\atcoin`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/atcoin
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/atcoin --strip
```
