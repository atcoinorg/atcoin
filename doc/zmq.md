...
The ATCOIN Core daemon can be configured to act as a trusted "border
router", implementing the atcoin wire protocol and relay, making
...
The ZeroMQ feature in ATCOIN Core requires the ZeroMQ API >= 4.0.0
...
For instance:
    ...
    $ atcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
...
terminator). These options can also be provided in atcoin.conf.
...
From the perspective of atcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into atcoind directly. Furthermore, no information is
...
using. A atcoind appends an up-counting sequence number to each
