UNIX BUILD NOTES
====================
Some notes on how to build ATCOIN Core in Unix.
...
memory available when compiling ATCOIN Core. On systems with less, gcc can be
...
To build ATCOIN Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
...
to: /path/to/atcoin/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/atcoin/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, ATCOIN Core can
