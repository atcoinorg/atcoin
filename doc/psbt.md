# PSBT Howto for ATCOIN Core

Since ATCOIN Core 0.17, an RPC interface exists for Partially Signed ATCOIN
...
PSBT is an interchange format for ATCOIN transactions that are not fully signed
...
Overall, the construction of a fully signed ATCOIN transaction goes through the
following steps:
...
- An **Extractor** produces a valid ATCOIN transaction (in network format)
...
#### Multisig with multiple ATCOIN Core instances
...
ATCOIN Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `atcoin-cli` is used.
