# Fuzzing ATCOIN Core using libFuzzer
...
## Overview of ATCOIN Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/bitcoin_report.pdf) includes an external overview of the status of ATCOIN Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.
...
It is possible to specify `atcoind` arguments to the `fuzz` executable.
...
an instrumentalized libc++, then using it to build ATCOIN Core dependencies
from source](../depends/README.md) and finally the ATCOIN Core fuzz binary
...
Every single pull request submitted against the ATCOIN Core repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make ATCOIN Core more robust.
...
# Fuzzing ATCOIN Core using Honggfuzz
...
ATCOIN Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/bitcoin-core)
...
ATCOIN Core follows its [security disclosure policy](https://bitcoincore.org/en/security-advisories/),
